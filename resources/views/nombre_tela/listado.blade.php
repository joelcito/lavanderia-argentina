...
@section('content')
    ...
    <div class="modal fade" id="modalNombreTela" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header" id="kt_modal_add_user_header">
                    <h3 class="fw-bold">FORMULARIO DE NOMBRE DE TELA <span class="text-info" id="nombre_busqueda"></span></h3>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body scroll-y">
                    <form id="formularioNombreTela">
                ...
                <div class="modal-footer">
                    <div class="row">
                        <div class="col-md-12">
                            <button class="btn btn-sm w-100 btn-success" onclick="guardarNombreTela()">Guardar</button>
                        </div>
                    </div>
                </div>
                <!--end::Modal body-->
...
<script src="{{ asset('assets/plugins/custom/datatables/datatables.bundle.js') }}"></script>
    <script>
        ...
        function ajaxListado(){
            let datos = {};
            $.ajax({
                url: "{{ route('nombre_tela.ajaxListado') }}",
                method: "POST",
                data: datos,
        ...
        function guardarNombreTela(){
            let datos = $('#formularioNombreTela').serializeArray();

             $.ajax({
                url: "{{ route('nombre_tela.guardarNombreTela') }}",
                method: "POST",
                data: datos,
                success: function(resultado) {
                    if (resultado.estado) {
        ...
        function editarNombreTela(nombre_tela){
        ...
        function eliminarNombreTela(nombre_tela, nombre) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('nombre_tela.eliminarNombreTela') }}",
                        method: "POST",
                        data: { nombre_tela: nombre_tela },
                        success: function(resultado) {
                            if (resultado.estado) {
